...
PyTorch implementation of [[https://papers.nips.cc/paper/7213-poincare-embeddings-for-learning-hierarchical-representations][Poincaré Embeddings for Learning Hierarchical Representations]] with modification and appenditure from Cassie Qiu and Sophia Sun.
...
** Running visualization, classification, and link prediction
We've made a few changes to the origional repositiory to enable visualization and link prediction (with K-fold cross validation and MAP score). View some example experiments in `visualization_classfication.ipynb` and `link_prediction.ipynb`  
